...
/// or the Tendask+ entitlement is missing it renders nothing (disabled feature,
/// not a swallowed error); the calendar itself is paid (spec §6.5). The "show
/// in journal" sub-switch governs the colour layer, not this entry
/// (screen-map §1.3).
...
class _JournalMoonButtonGate extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    if (!ref.watch(plusActiveProvider)) return const SizedBox.shrink();
...
/// [month], or null while the layer is off (feature flag, entitlement or the
/// "show in journal" sub-switch — board C). Null keeps the grid pixel-identical
/// to the moonless journal.
