...
| **FR-22** | Brez lokacije vremena ne kažemo — na njegovem mestu kartica s povezavo | `gardenLocation` vrne `GardenCoords?`; UI gleda **lokacijo, ne celice** (pokrije tudi shranjeno celico, ki je neuporabna — `cellCentroid` vrne null). Brez lokacije ni klica na Open-Meteo in ni zamrznjenega posnetka. Kartica ~72 dp: `kNoWeatherEmoji`, naslov o vremenu, ena poved s povezano besedo prek slang `(rich)`; tarča je cela kartica, zato span nima `TapGestureRecognizer` in widget ostane `StatelessWidget`. Isto pri opravilu, ki čaka, in v povzetku vnosa | Privzetek Ljubljana je tuje vreme kazal kot uporabnikovo in ga ob zaključku **zamrznil v dnevnik za vedno** — to je bilo resnejše od napačne kartice, ker se posnetek nikoli ne osveži. Prva izvedba povabila (pin, gumb, zasebnostna vrstica, ~168 dp) je izgubila vez z vremenom, zato kartica zdaj govori o vremenu, ne o lokaciji. Zaključeno opravilo brez posnetka **ne dobi razlage**: task ne hrani, zakaj ga ni, trenutna lokacija pa o preteklosti ne pove nič. `(rich)` namesto treh ključev, ker deljena poved razbije nemški besedni red. `kDevLatitude/kDevLongitude` ostaneta samo `--dart-define` in ju nobena defines datoteka ne nastavlja → v izdaji `null`. **Neizdano.** |
| **FR-24** | Onboarding lokacija: poudarek na GPS, ne na preskoku | Poudarjena GPS kartica na dnu + nepoudarjen »Preskoči«; z nastavljeno lokacijo si poudarjenost zamenjata (»Nadaljuj«). Potrditev shranjevanja nosi statusni trak, ne toast | Edina polna zelena ploskev na zaslonu je bila doslej gumb, ki lokacijo **preskoči** — 52 od 95 profilov je brez nje. **Blokade ni:** lokacija ostane prostovoljna, preskok en tap; hierarhija samo pove, kaj je pričakovano. Toast je prekrival prav trak, ki isto pove in ostane. Merilo: 25 % → **≥60 %** novih z `h3_r5` v 30 dneh. **Neizdano.** |
| **FR-25** | Vreme pri opombah: obljuba umaknjena, zajema ni | Ključ `notes.info` (sl/en/de) in `_InfoHint` iz obrazca za opombo ven, ista poved tudi iz `wireframes/18-note-edit.html`. Stolpec `note.weather` **ostane** — shema je add-only in sync ga že prenaša v obe smeri | Opombe vremena niso zajemale **nikoli** — `git log -S weather -- lib/features/journal/` je prazen. Zajem je poceni samo za opombo z datumom **danes**: datum opombe je uporabnikov (izbirnik 2020 … +365 dni), `buildSnapshot` pa je zasidran na »zdaj« (pas 1 bere `res.current`, pas 3 šteje dneve naprej). Open-Meteo bi za nazaj dal 92 dni prek `past_days`, starejše šele ERA5 arhiv (5 dni zamika, 25 km) — dva vira v istem stolpcu. Pošten model je `f(h3_r7, dan)` iz FR-7, kjer opomba vreme **poizve** namesto zamrzne; do takrat je molk boljši od obljube. Zajem »ob sprožitvi opomnika« je **nemogoč**, ne le drag: `zonedSchedule` preda obvestilo AlarmManagerju in ob prikazu se Dart koda ne izvede — plugin ima povratni klic samo na uporabnikov tap. Obljuba pri **opravilih** (`entry.weather_note`, onboarding) ostane: tam je resnična. |
