...
- **Glifi, ikone in animacije imajo svoje kataloge — widget ne piše literala.** Emoji → `core/glyphs.dart` (`kGlyph*`; domenske mape kot `elementEmoji()`/`areaTypeIcon()` berejo od tam), Material ikone → `core/app_icons.dart` (`kIcon*`, imena zrcalijo Material), trajanja animacij → `app/theme/app_motion.dart` (`kMotionFast/Medium/Slow`), barve → `AppColors`. Razlog (sweep 2. 8. 2026): isti pomen na dveh zaslonih se je razšel — `🌿` kot fallback rastline je živel na 8 mestih, `📋` na 4.
...
- **Dneve premikaj s `addDays()`** (`core/date_format.dart`), **nikoli z `add(Duration(days: n))`** nad lokalnim `DateTime`. Lokalni dan traja 23 ali 25 ur (premik ure): 24-urni bloki ob jesenskem premiku pristanejo dvakrat na istem datumu in enega preskočijo. Ujeto v pregledu 2. 8. 2026 — lunin namig je za isti dan načrtoval dve enaki obvestili, horizont pa izgubil zadnji dan; isti vzorec je bil še v dnevniškem nudgeu, opomnikih, ponovitvi opravila (»+1 dan«) in oznakah »jutri/včeraj«. Absolutni odmiki (»30 minut prej«) ostanejo `Duration`.
...
- **`StreamProvider` teče samo, dokler ga kdo posluša** (Riverpod 3; ujeto 3. 8. pri `plusProvider`). `ref.listen`/`ref.watch` **iz providerja, ki ga sam nihče ne posluša** (npr. koordinator, zagnan prek `container.read(p.notifier).start()`) streama **ne** oživi — vrednost obtiči na `AsyncLoading` in `.future` nikoli ne dokonča. Kdor tak stream **bere** (`ref.read`) namesto watcha, rabi **odprt** poslušalec (`container.listen(p, (_, _) {})`, ki ga NE zapre): ob zaprtju zadnjega poslušalca provider obdrži zadnjo vrednost, a nove ne dobiva več. V testih isti vzorec + `await pumpEventQueue()`.
