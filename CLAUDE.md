...
Vir resnice za koncept/design: `docs/koncept.md` (§7.9 entiteta opravilo, §7.14 podatkovni model), `docs/tech-stack.md` (potrjen sklad + §2 sync arhitektura + §6 struktura + §10 konvencije), `docs/brand/brand.md` (vizualna identiteta), `docs/opravila-in-rastline.md` (vir za seed kataloga). Razvojni plan + delovni dogovor: `docs/roadmap.md` (M0–M11). Ta dokument pokriva **kako** piševa, ne **kaj** gradiva.

## Vodilna načela (po prioriteti)

1. **Offline-first — drift je vir resnice za UI.** Lokalna drift (SQLite) baza je vir resnice, ki ga bere in piše UI; Supabase je vir resnice v oblaku. **UI vedno piše v drift, nikoli direktno v Supabase** — sync poskrbi za oblak v ozadju. Aplikacija mora delovati brez signala (vrt!).
2. **Zasebnost po zasnovi.** Samo H3 celice, nikoli surove koordinate; RLS povsod; skrivnosti prek `--dart-define`; GDPR izvoz + izbris. Podrobnosti → `## Zasebnost in varnost`.
3. **Preproste delujoče rešitve, brez over-engineeringa.** Ne uvajaj abstrakcij, dokler nimaš ≥3 konkretnih klicalnikov. Brez "future-proof" tovarn, brez generikov za en tip, brez interface-a za en implement. Tri podobne vrstice so boljše od prezgodnje abstrakcije.
   ...
   - **Abstrahiraj samo stabilen API.** Workflow v nastajanju (npr. sync push/pull ali UI obrazec, dokler ni potrjen) pusti duplicirano — abstrakcija nad gibljivo tarčo postane gnoj.
   - **Eksplicitna koda pred implicitno magijo.** Ne skrivaj business logike v extension-ih ali generic helperjih; bralec mora videti, *kaj* se zgodi. Extension-i so OK za syntactic sugar nad tujimi tipi (`BuildContext.theme`), ne za pravila aplikacije.
...
## Tendask stil (hitri filter)

Pred vsakim večjim kosom kode — če katero od teh ne drži, se ustavi in premisli:
...
> **Code-gen ni magija, ki bi se ji izogibali:** Riverpod generator, freezed, drift, slang, json_serializable so del potrjenega sklada. Pravilo "minimalna magija" velja za *ročno* napisano kodo (refleksija, dynamic dispatch, exotic operatorji), ne za generirane razrede.
...
- **Repo ne sprejema/vrača drift tipov na meji.** Metode so tipirane (`create(...)`, `updateTask({...})`) — **nikoli `Companion`/`Value` v podpisu**; `package:drift` ostane v `data/`. Widget nikoli ne sestavi `Companion` (glavni zdrs M2).
...
- **Začasni outputi gredo v `tmp/`** v korenu (analyze dump, log capture, scratch JSON, začasni SQL) — nikoli razmetani po repu. Če `tmp/` ne obstaja, jo najprej ustvarim; je v `.gitignore`.
...
- **Vse magične vrednosti v `core/config.dart` ali ustreznem `theme/` filu.** Velja za: sync interval, weather cache TTL, privzeti zamik opomnika, tihe ure, frekvenčno kapico, H3 resolucije (r7/r6/r5), agronomske pragove (frost-anchor) za pametni motor, barve, paddinge, animation duration.
...
- **State management: Riverpod (s code-gen) od M0.** `@riverpod` anotacija, `ref.watch` v `build`, `ref.read` v callback. Provider scope = global root.
...
- **Zero `print()` v production kodi.** Uporabi `debugPrint` (auto-strip v release), za strukturirano `package:logging`. V release crash/napake → Sentry.
...
- **`!` (bang) operator z resnico.** Nikoli `foo!` brez komentarja v eni vrstici nad njim, ki pojasni **zakaj** je `null` tu nemogoč. Privzeto: `if (foo == null) return;` ali `switch (foo) { case null => ...; case final v => ... }`.
- **`analysis_options.yaml` je executable verzija teh pravil.** Baseline `package:flutter_lints/flutter.yaml`; treat-as-error: `avoid_print`, `unawaited_futures`, `use_build_context_synchronously`, `prefer_const_constructors`, `prefer_const_literals_to_create_immutables`. Lint, ki se da avtomatizirati, NE sme živeti samo v CLAUDE.md.

## UI / presentation vzorci (naučeno iz pregleda M2)

**Pred delom na zaslonu/obrazcu/sheetu preberi `docs/ui-katalog.md`** — komponentni katalog (en widget na vzorec: `SectionLabel`, `FieldLabel`, `EmptyState`, `DestructiveButton`, `SaveBar`, `SheetHandle`, `showConfirmDialog`) in pravila barv prek teme živijo tam.

- **Brez podvojenih widgetov.** Če isti widget (tile, save-bar, dialog, sheet-handle, empty-state) rabita ≥2 zaslona → `core/widgets/` (generičen) ali `features/<x>/presentation/widgets/` (feature-specifičen). **Verbatim copy-paste widgeta med zasloni je rdeč alarm** že pri 2 klicalcih (izjema od ≥3 pravila).
- **Ne prenašaj `theme`/`t` skozi konstruktorje.** `Theme.of(context)` in `context.t` sta poceni `InheritedWidget` lookup-a — beri ju lokalno v vsakem widgetu (prop-drilling = šum v vsakem podpisu).
- **Katalog labels samo prek `catalogLabel()`** (`core/catalog_labels.dart`) — nikoli ročni `jsonDecode` labels v widgetu (splošno: brez `Map<String,dynamic>` v presentation plasti).
- **Datumi za prikaz prek `core/date_format.dart`** (`startOfDay`/`formatDmy`/`formatHm`), ne podvojeni inline izračuni po zaslonih.
- **Napake niso `SizedBox.shrink()`.** Tiho požiranje skrije bug — pokaži vsaj miren indikator (kasneje Sentry). Mreža = pričakovano stanje (graceful degrade); lokalni DB error = bug.
- **Velika presentation datoteka (>~300 vrstic) je signal za ekstrakcijo widgetov**, ne za scroll.

## Zasebnost in varnost

- **H3 celico računaj na napravi** (`h3_flutter`), shrani `profile.h3_r7/r6/r5`; **koordinate nikoli ne zapustijo naprave** in se nikoli ne shranijo.
- **RLS povsod:** uporabniške tabele `user_id = auth.uid()`; katalog (`task_type`, `plant`, …) javno-bralen. Anonimni uporabnik = veljaven `auth.uid()`, RLS deluje enako.
- **Skrivnosti prek `--dart-define`** (Supabase `url`+`anonKey`, Sentry DSN) — nikoli v repo. `.env`, keystore-ji, `key.properties`, `local.properties` → `.gitignore`. Anon key + RLS = dovolj; brez service-role ključev v aplikaciji.
...
- **LWW po `updated_at`.** Vsaka uporabniška vrstica ima `id` (UUID, **generiran na napravi** pred vstavljanjem), `updated_at` (timestamptz), `deleted` (bool, **soft delete** — da se izbris sinhronizira), in lokalno (samo drift) `sync_status ∈ {pending, synced}`.
...
- **Repo normalizira čas v UTC.** Klicalec poda lokalni `DateTime`; repo naredi `.toUtc()` pred zapisom (isti `create`/`updateTask` v dveh widgetih ne sme dati različnih rezultatov). Za prikaz `.toLocal()` prek `core/date_format.dart`.
- **Domenska stanja = `enum` prek drift `textEnum`** (npr. `TaskStatus { waiting, done }`), shranjen kot `enum.name` → zrcalno s Supabase, brez migracije. Query: `col.equalsValue(Enum.x)`. **Gotcha:** glavni `app_database.dart` mora **importati enum**, sicer generirani `*.g.dart` (part-of) javi "Type not found" (`flutter analyze` tega ne ujame, `flutter test` ga). Workflow v nastajanju pusti `String`.
- **Shema drift in Supabase morata zrcaliti.** Ob spremembi tabele posodobi oba. **Migracije = additive-only:** add column/table = OK; rename = NIKOLI (add new + dual-write + drop čez 2 release); nov column = nullable ali z default (nikoli `NOT NULL` brez backfilla v isti migraciji). Razlog: stari APK-ji ob pull-u ne smejo crash-ati.
- **Katalog (`task_type`, `plant`, …): oblak je vir resnice, naprave ga pull-ajo (od M6); klient kataloga NE piše** (RLS read-only). Vsebinski vir `lib/data/seed/catalog_seed.dart` → `tool/gen_catalog_sql.dart` → `supabase/seed/catalog.sql` (idempotenten `on conflict do update`), aplicirano prek service role/pooler (`supabase/seed/apply_catalog.py`). **Katalog id-ji (`task_type.id`, `plant.id`) so add-only — NIKOLI preimenuj/izbriši** id, ki je v rabi: `user_plant.plant_id`/`task.task_type_id` kažeta nanje in ju preimenovanje osiroti. Ob širjenju (Wikidata/GBIF) vzemi stabilen ključ ali ohrani obstoječ slug. **Bundlan seed (128 vrst) je offline fallback za prvi zagon brez signala** (on-device potrjeno, M9.6).
...
- **Vreme (Open-Meteo):** ob izvedbi opravila zajemi posnetek; če mreže ni, opravilo se vseeno shrani. **Zamrznjen dejanski posnetek na "opravljeno" se ne prepisuje.**
- **Connectivity status je explicit signal** prek enega `connectivity_plus` streama, ki ga features konzumirajo.
- **Nobenega runtime fetcha sredstev (fonti, ikone, asseti).** Vse bundlano v `assets/`. Naučeno: `google_fonts` privzeto **naloži font prek omrežja** (`fonts.gstatic.com`) → offline (vrt!) vrže Unhandled Exception. Plus Jakarta Sans je bundlan variable font (`assets/fonts/`, `pubspec fonts:`), uporabljen prek `fontFamily` v `app_theme`, ne prek `GoogleFonts.*`.

## Jezik, komentarji, dokumentacija

**Jezikovno pravilo (fiksno):** pogovor agent ↔ razvijalec je **slovenščina**, **vsa koda angleščina**. Koda zajema: identifikatorje, route poti in imena (`/journal` ne `/dnevnik`; `name: 'journal'`), i18n ključe (`nav.journal`), log/error nize, asset poti, imena datotek, komentarje in doc komentarje. Slovenščina živi samo v `docs/*`, pravilih (`CLAUDE.md`, `docs/roadmap.md`) in klepetu — **nikoli v kodi**.
...
- **Jedrnato — en stavek.** Brez opisa toka (`// calls X then Y`), brez reference na kličoče (`// used by Z`) — to spada v PR opis.
...
- **En korak iz `docs/roadmap.md` = en commit.** Conventional Commits: `feat:`, `fix:`, `refactor:`, `chore:`, `docs:`, `test:`. Naslov ≤72 znakov, brez pike, slovenski opis; body razloži *zakaj*, ne *kaj*.
...
- Vsak nov package: preveri pub.dev score, zadnji release datum, issue tracker. Mrtvih lib-ov ne dodajaj. **Drži se potrjenega sklada v `tech-stack.md` §1** — paket izven seznama = najprej vprašaj.
...
- **Pure logika** (drift poizvedbe, sync LWW/vrstni red, Open-Meteo parser, motor pravil): unit testi obvezni. Tečejo na CI brez naprave.
...
- **Layout regresije: `test/layout/` matrika** (`layout_harness.dart` + `layout_matrix_test.dart`). Vsak zaslon se izriše čez viewport × locale × text-scale (320/360/411 × sl/en/de × 1.0/1.3) in lovi overflow + odrezan tekst — ujame prelome pri dolgi nemščini in veliki pisavi. **Pomni:** prosto-ovijajoč tekst (`softWrap && maxLines==null`) se nikoli ne odreže (Flutter razlomi tudi predolgo besedo), zato `getMinIntrinsicWidth` NI signal za odrez — flag samo vrstično omejen tekst. Nov zaslon dodaš z enim `layoutMatrix(...)` klicem.
...
- Dodajati novo dependency (izven `tech-stack.md` §1); spreminjati `pubspec.lock` ali `gradle` verzij.
