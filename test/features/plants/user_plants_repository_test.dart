...
void main() {
  ...
  group('recentPlantIds', () {
    test(
      'distinct species, newest first, excludes custom + deleted + limit',
      () async {
  ...
  group('update', () {
    test('changes area, keeps the passed alias, marks pending', () async {
      ...
      await (db.update(db.userPlants)..where((p) => p.id.equals(id))).write(
        const UserPlantsCompanion(syncStatus: Value(kSyncSynced)),
      );
